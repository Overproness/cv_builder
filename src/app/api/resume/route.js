...
export async function POST(request) {
  ...
  try {
    ...
    const { title, company, position, jobDescription, latex, tailoredCV, masterCVId } = body;
    ...
    const resume = await Resume.create({
      ...
      title: title || (company && position ? `${position} at ${company}` : 'Tailored Resume'),
      company: company || '',
      position: position || '',
    ...
    return NextResponse.json({ message: 'Resume saved', id: resume._id.toString() });
  } catch (error) {
